...
export class PayrollConfigurationService {
  ...
  private async validateSalaryRangeOverlap(
    ...
    const overlappingBracket = await this.insuranceBracketsModel.findOne(
      query,
    );
  ...
  async updateInsuranceBracket(
    ...
    const newEmployeeRate =
      updateInsuranceBracketDto.employeeRate ??
      insuranceBracket.employeeRate;
    const newEmployerRate =
      updateInsuranceBracketDto.employerRate ??
      insuranceBracket.employerRate;
  ...
  async findAllSigningBonuses(
    ...
    if (filter?.minAmount !== undefined) {
      query.amount = { ...((query.amount as object) || {}), $gte: filter.minAmount };
    }
    if (filter?.maxAmount !== undefined) {
      query.amount = { ...((query.amount as object) || {}), $lte: filter.maxAmount };
    }
   ...
   * @author John Wasfy
   */
  async submitSigningBonusForApproval(id: string): Promise<signingBonusDocument> {
    const signingBonus = await this.findSigningBonusById(id);
  ...
  /**
   * Get all approved signing bonuses (for use in payroll execution)
   * 
   * @author John Wasfy
   */
  ...
  async validateEligibility(
    ...
    const isFullTime = workType === 'FULL_TIME' || workType === 'Full Time Employees';
  ...
  validateManualOverride(amount: number, limit: number): void {
    if (amount > limit) {
      throw new BadRequestException(`Manual override amount ${amount} exceeds limit ${limit}`);
    }
  }
   ...
   */
  validateContributionPercentages(employeeRate: number, employerRate: number): void {
   ...
   */
  validateApprovalStatus(currentStatus: string, action: 'approve' | 'reject'): void {
    if (currentStatus === ConfigStatus.APPROVED) {
      throw new BadRequestException(`Cannot ${action}. Entity is already approved.`);
    }
    if (currentStatus === ConfigStatus.REJECTED && action === 'reject') {
      throw new BadRequestException(`Cannot ${action}. Entity is already rejected.`);
    }
  }
