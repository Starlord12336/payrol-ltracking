...
import { Document as DocModel, DocumentDocument } from './models/document.schema';
...
import { CreateOnboardingTaskDto, UpdateTaskStatusDto } from './dto/onboarding-task.dto';
...
import { InitiateTerminationReviewDto, PerformanceDataForReviewDto } from './dto/initiate-termination-review.dto';
import {
...
export class RecruitmentService {
	...
	private templatesDir = path.join(process.cwd(), 'data', 'hiring-process-templates');
	private assessmentFormsDir = path.join(process.cwd(), 'data', 'assessment-forms');
	private assessmentResponsesDir = path.join(process.cwd(), 'data', 'assessment-responses');
	private panelMembersDir = path.join(process.cwd(), 'data', 'panel-members');
	private panelAvailabilityDir = path.join(process.cwd(), 'data', 'panel-availability');
	...
	private assessmentResponsePath(interviewId: string, fileName: string) {
		return path.join(this.assessmentResponsesDir, String(interviewId), fileName);
	}
	...
	private consentPath(candidateId: string, fileName: string) {
		...
		async saveProcessTemplate(key: string, payload: { name: string; stages: string[] }) {
		...
		async saveAssessmentForm(key: string, payload: { name: string; role?: string; positionCode?: string; criteria: Array<{ key: string; label: string; weight?: number }> }) {
		...
		async readAssessmentForm(key: string) {
			...
			try {
				const txt = await fsp.readFile(file, 'utf8');
				return JSON.parse(txt) as { name: string; role?: string; positionCode?: string; criteria: Array<{ key: string; label: string; weight?: number }> };
			} catch (err) {
				return null;
			}
		}
		...
		async listAssessmentForms() {
			...
			const result: Array<{ key: string; name: string; role?: string; criteriaCount: number }> = [];
			for (const f of files) {
				if (!f.endsWith('.json')) continue;
				const key = f.replace(/\.json$/, '');
				const t = await this.readAssessmentForm(key);
				if (t) result.push({ key, name: t.name, role: t.role, criteriaCount: t.criteria?.length ?? 0 });
			}
			return result;
		}
		...
		async submitStructuredAssessment(interviewId: string, interviewerId: string, formKey: string, responses: Record<string, number>, comments?: string) {
			...
			const avg = count === 0 ? null : Math.round((weightedSum / (totalWeight || count)) * 100) / 100;

			// ensure responses dir for this interview
			await this.ensureAssessmentResponsesDir();
			const interviewDir = path.join(this.assessmentResponsesDir, String(interview._id));
		...
		async listStructuredResponses(interviewId: string) {
			...
			try {
				...
				const out: Array<{ file: string; path: string; summary?: { interviewerId?: string; computedScore?: number; submittedAt?: string } }> = [];
				for (const f of files) {
					if (!f.endsWith('.json')) continue;
					...
					try {
						...
						out.push({ file: f, path: path.relative(process.cwd(), p), summary: { interviewerId: obj.interviewerId, computedScore: obj.computedScore, submittedAt: obj.submittedAt } });
					} catch (e) {
		...
		async createReferral(referringEmployeeId: string, candidateId: string, opts?: { role?: string; level?: string }) {
		...
		async prioritizeApplication(applicationId: string, opts?: { changedBy?: string; expedite?: boolean; expediteToStage?: string }) {
			const app = await this.applicationModel.findById(applicationId).exec();
			if (!app) throw new NotFoundException('Application not found');
			const referrals = await this.getReferralsForCandidate(String(app.candidateId));
			const isReferred = referrals && referrals.length > 0;

			const payload: any = { applicationId: String(app._id), isReferred, referralsCount: referrals.length };
			...
			if (opts?.expedite) {
				...
				await this.advanceApplicationStage(String(app._id), { newStage: target, changedBy: opts?.changedBy || String(primary.referringEmployeeId) });
		...
		async saveConsent(candidateId: string, granted: boolean, type = 'personal', givenBy?: string, details?: string) {
		...
		async listConsents(candidateId: string) {
			...
			try {
				...
				for (const f of files) {
					...
					try {
						const txt = await fsp.readFile(p, 'utf8');
						out.push({ file: f, path: path.relative(process.cwd(), p), data: JSON.parse(txt) });
					} catch (e) {
						out.push({ file: f, path: path.relative(process.cwd(), p) });
					}
				}
	...
	async previewJobRequisition(requisitionId: string) {
		const requisition = await this.jobRequisitionModel.findById(requisitionId).lean().exec();
		if (!requisition) throw new NotFoundException('Job requisition not found');

		let template: any = null;
		if (requisition.templateId) {
			template = await this.jobTemplateModel.findById(requisition.templateId).lean().exec();
		}
		...
		const brand = {
			...
			agencyTagline: process.env.COMPANY_TAGLINE || 'Join our team and make an impact.',
		};
	...
	async publishJobRequisition(requisitionId: string, opts?: { expiryDays?: number }) {
		const requisition = await this.jobRequisitionModel.findById(requisitionId).exec();
		if (!requisition) throw new NotFoundException('Job requisition not found');
	...
	async uploadCandidateCV(candidateId: string, file: { path: string; originalname?: string }) {
	...
	async applyToRequisition(candidateId: string, requisitionId: string, opts?: { documentId?: string }) {
	...
	async advanceApplicationStage(applicationId: string, opts?: { newStage?: string; changedBy?: string; templateKey?: string }) {
		let defaultStages = ['Screening', 'Shortlisting', 'Interview', 'Offer', 'Hired'];
		if (opts?.templateKey) {
		...
		const lastStageName = opts?.newStage ?? lastHistory?.newStage ?? mapStageEnumToName(application.currentStage);
		...
		const changedBy = opts?.changedBy || (application as any).assignedHr || application._id;
		...
		const isHired = typeof nextStageName === 'string' && nextStageName.toLowerCase() === 'hired';
		if (isHired) {
		...
		return { applicationId: application._id, newStage: nextStageName, progress };
	}

	// Get current status and progress for an application
	async getApplicationStatus(applicationId: string) {
		const defaultStages = ['Screening', 'Shortlisting', 'Interview', 'Offer', 'Hired'];
		const application = await this.applicationModel.findById(applicationId).lean().exec();
		...
		const currentStageName = mapStageEnumToName(application.currentStage) ?? lastHistory?.newStage ?? null;
		const stageIndex = currentStageName ? Math.max(0, defaultStages.indexOf(currentStageName)) : -1;
		const progress = stageIndex === -1 ? 0 : Math.round(((stageIndex + 1) / defaultStages.length) * 100);
	...
	async listApplicationsForCandidate(candidateId: string) {
		const apps = await this.applicationModel.find({ candidateId }).lean().exec();
		return apps.map((a) => ({ id: a._id, requisitionId: a.requisitionId, currentStage: a.currentStage, status: a.status }));
	}

	// Get application status history
	async getApplicationHistory(applicationId: string) {
		return this.applicationHistoryModel.find({ applicationId }).sort({ createdAt: 1 }).lean().exec();
	}
	...
	async prepareCandidateNotification(applicationId: string) {
		...
		const candidate = await this.candidateModel.findById(application.candidateId).lean().exec();
		if (!candidate) throw new NotFoundException('Candidate not found');
	...
	private rejectionTemplates: Record<string, { subject: string; body: string }> = {
		default: {
			subject: 'Application Update from ' + (process.env.COMPANY_NAME || 'Our Company'),
			body:
	...
	async prepareRejectionNotification(applicationId: string, opts?: { templateKey?: string; reason?: string }) {
		const application = await this.applicationModel.findById(applicationId).lean().exec();
		if (!application) throw new NotFoundException('Application not found');
		...
		const template = this.rejectionTemplates[templateKey] || this.rejectionTemplates.default;
	...
	async rejectApplication(applicationId: string, opts?: { templateKey?: string; reason?: string; changedBy?: string }) {
		const application = await this.applicationModel.findById(applicationId).exec();
		if (!application) throw new NotFoundException('Application not found');
		...
		const payload = await this.prepareRejectionNotification(applicationId, { templateKey: opts?.templateKey, reason });
	...
	async scheduleInterview(applicationId: string, opts: {
		stage: string;
		...
		const scheduledDate = typeof opts.scheduledDate === 'string' ? new Date(opts.scheduledDate) : opts.scheduledDate;
		...
		await this.applicationHistoryModel.create({
			...
			oldStatus: application.status ?? null,
			newStatus: application.status ?? null,
			changedBy: opts.createdBy || (application as any).assignedHr || application._id,
		});
		...
		if (candidate?.personalEmail) attendees.push({ email: candidate.personalEmail, name: candidate.fullName || candidate.firstName });
		if (opts.panelEmails) for (const e of opts.panelEmails) attendees.push({ email: e });
		...
		const invite = {
			...
			method: opts.method || InterviewMethod.VIDEO,
			videoLink: opts.videoLink || (opts.method === InterviewMethod.VIDEO ? `https://video.example.com/${String(interview._id)}` : undefined),
			attendees,
	...
	async saveAssessmentResult(interviewId: string, interviewerId: string, score: number, comments?: string) {
		const interview = await this.interviewModel.findById(interviewId).exec();
		if (!interview) throw new NotFoundException('Interview not found');
		...
		const all = await this.assessmentResultModel.find({ interviewId: interview._id }).lean().exec();
		const total = all.length;
		const avg = total ? Math.round((all.reduce((s: number, r: any) => s + (r.score || 0), 0) / total) * 100) / 100 : null;
		...
		const payload = {
			...
			aggregate: { averageScore: avg, submissions: total, panelCount: (interview.panel || []).length },
	...
	async getInterviewFeedbackSummary(interviewId: string) {
		const interview = await this.interviewModel.findById(interviewId).lean().exec();
		if (!interview) throw new NotFoundException('Interview not found');
		const feedbacks = await this.assessmentResultModel.find({ interviewId: interview._id }).lean().exec();
		const total = feedbacks.length;
		const avg = total ? Math.round((feedbacks.reduce((s: number, r: any) => s + (r.score || 0), 0) / total) * 100) / 100 : null;
		return {
			interview,
			feedbacks,
			aggregate: { averageScore: avg, submissions: total, panelCount: (interview.panel || []).length },
	...
	async getRecruitmentDashboard() {
		...
		const requisitions = await this.jobRequisitionModel.find({ publishStatus: 'published' }).lean().exec();
		const defaultStages = ['Screening', 'Shortlisting', 'Interview', 'Offer', 'Hired'];
		...
		for (const r of requisitions) {
			...
			let template: any = null;
			if (r.templateId) template = await this.jobTemplateModel.findById(r.templateId).lean().exec();

			const apps = await this.applicationModel.find({ requisitionId: r._id }).lean().exec();
			...
			let progressCount = 0;
			for (const a of apps) {
				const lastHistory = await this.applicationHistoryModel.findOne({ applicationId: a._id }).sort({ createdAt: -1 }).lean().exec();
				const currentStageName = mapStageEnumToName(a.currentStage) ?? lastHistory?.newStage ?? null;
				const stageIndex = currentStageName ? Math.max(0, defaultStages.indexOf(currentStageName)) : -1;
				const progress = stageIndex === -1 ? 0 : Math.round(((stageIndex + 1) / defaultStages.length) * 100);
			...
			const avgProgress = progressCount === 0 ? 0 : Math.round((progressSum / progressCount) * 100) / 100;
		...
		const overallAvgProgress = totalProgressCount === 0 ? 0 : Math.round(((totalProgressSum / totalProgressCount) * 100)) / 100;
	...
	async createOffer(payload: {
		...
		const app = await this.applicationModel.findById(payload.applicationId).exec();
		if (!app) throw new NotFoundException('Application not found');
		const candidate = await this.candidateModel.findById(payload.candidateId).lean().exec();
		if (!candidate) throw new NotFoundException('Candidate not found');
	...
	async addOfferApproverAction(offerId: string, employeeId: string, role: string, status: string, comment?: string) {
		const offer = await this.offerModel.findById(offerId).exec();
		if (!offer) throw new NotFoundException('Offer not found');

		const action = { employeeId, role, status, actionDate: new Date(), comment } as any;
	...
	async finalizeOffer(offerId: string) {
		...
		const anyRejected = approvers.some((a: any) => String(a.status).toLowerCase() === 'rejected' || String(a.status).toLowerCase() === 'rejected');
		...
		if (app) {
			if (final === 'approved') {
				...
				await app.save();
				await this.applicationHistoryModel.create({ applicationId: app._id, oldStage: null, newStage: 'Offer Finalized: Approved', oldStatus: null, newStatus: ApplicationStatus.HIRED, changedBy: null });
			} else {
				app.status = ApplicationStatus.REJECTED;
				await app.save();
				await this.applicationHistoryModel.create({ applicationId: app._id, oldStage: null, newStage: 'Offer Finalized: Rejected', oldStatus: null, newStatus: ApplicationStatus.REJECTED, changedBy: null });
			}
		}
	...
	async candidateRespondOffer(offerId: string, response: 'accepted' | 'rejected') {
		const offer = await this.offerModel.findById(offerId).exec();
		if (!offer) throw new NotFoundException('Offer not found');
		...
		await this.applicationHistoryModel.create({ applicationId: offer.applicationId, oldStage: null, newStage: `Offer Responded: ${response}`, oldStatus: null, newStatus: null, changedBy: String(offer.candidateId) });
	...
	async generateOfferLetter(offerId: string, options?: { template?: string }) {
		...
		if (!offer) throw new NotFoundException('Offer not found');
		const candidate = await this.candidateModel.findById(String(offer.candidateId)).lean().exec();
		...
		const mapping: Record<string, any> = {
			candidateName: cand?.name || (cand?.firstName ? `${cand.firstName} ${cand.lastName || ''}` : 'Candidate'),
			role: offer.role || '',
			...
			deadline: offer.deadline ? new Date(offer.deadline).toLocaleDateString() : '',
		...
		return { letter, meta: { offerId, candidateId: String(offer.candidateId) } };
	}
	...
	async prepareOfferSendPayload(offerId: string) {
		...
		const to = (candidate as any)?.email || (candidate as any)?.contactEmail || null;
	...
	async submitOfferSignature(offerId: string, signature: { type: 'image' | 'typed'; data: string; signerName?: string; signedAt?: string }) {
		const offer = await this.offerModel.findById(offerId).exec();
		if (!offer) throw new NotFoundException('Offer not found');

		const candidateId = String(offer.candidateId);
		const basePath = path.join(process.cwd(), 'data', 'offer-signatures', offerId);
		...
		if (signature.type === 'image') {
			...
			signatureFile = path.join(basePath, filename);
			await fs.promises.writeFile(signatureFile, JSON.stringify({ signerName: signature.signerName || null, typed: signature.data, signedAt: signature.signedAt || new Date().toISOString() }, null, 2));
		}

		// update offer record: mark as accepted and set signed timestamp
		offer.applicantResponse = 'accepted' as any;
		offer.candidateSignedAt = signature.signedAt ? new Date(signature.signedAt) : new Date();
		await offer.save();

		// record history (changedBy: candidateId required by schema - try to use candidateId)
		try {
			await this.applicationHistoryModel.create({ applicationId: offer.applicationId, oldStage: null, newStage: 'Offer Signed', oldStatus: null, newStatus: null, changedBy: candidateId });
		} catch (e) {
	...
	async triggerPreboarding(offerId: string, options?: { startDate?: string; tasks?: Array<{ title: string; description?: string; assignee?: 'candidate' | 'hr'; dueDays?: number }> }) {
		const offer = await this.offerModel.findById(offerId).exec();
		if (!offer) throw new NotFoundException('Offer not found');

		// ensure offer accepted
		const accepted = (String(offer.applicantResponse || '').toLowerCase() === 'accepted') || !!offer.candidateSignedAt;
		if (!accepted) throw new Error('Offer not accepted; cannot trigger pre-boarding');
		...
		const defaultTasks = [
			{ title: 'Sign employment contract', description: 'Electronically sign the official employment contract.', assignee: 'candidate', dueDays: 3 },
			{ title: 'Complete tax and payroll forms', description: 'Provide tax information and bank details for payroll.', assignee: 'candidate', dueDays: 5 },
			{ title: 'Upload ID documents', description: 'Upload government-issued ID for verification.', assignee: 'candidate', dueDays: 5 },
			{ title: 'Benefits enrollment', description: 'Choose benefit options and enroll.', assignee: 'candidate', dueDays: 7 },
			{ title: 'IT access & equipment request', description: 'Request laptop, accounts and access rights.', assignee: 'hr', dueDays: 10 },
			{ title: 'Payroll setup', description: 'HR to configure payroll for the new hire.', assignee: 'hr', dueDays: 10 },
		];

		const tasksSource = options?.tasks && options.tasks.length ? options.tasks : defaultTasks;
		...
		for (let i = 0; i < tasksSource.length; i++) {
			...
			const id = `${ts}-${i}`;
			tasks.push({ id, title: t.title, description: t.description || null, assignee: t.assignee || 'candidate', dueDate: due.toISOString(), status: 'pending', createdAt: new Date().toISOString() });
		}

		const filePath = path.join(basePath, 'tasks.json');
		await fsp.writeFile(filePath, JSON.stringify({ offerId, createdAt: new Date().toISOString(), tasks }, null, 2), 'utf8');

		// record history entry on application
		try {
			await this.applicationHistoryModel.create({ applicationId: offer.applicationId, oldStage: null, newStage: 'Pre-boarding Started', oldStatus: null, newStatus: null, changedBy: offer.hrEmployeeId || null });
		} catch (e) {
			// ignore best-effort
		}

		return { offerId, tasksFile: path.relative(process.cwd(), filePath), tasks };
	}
	...
	async completePreboardingTask(offerId: string, taskId: string, completedBy?: string) {
		const basePath = path.join(process.cwd(), 'data', 'preboarding', offerId);
		const filePath = path.join(basePath, 'tasks.json');
		try {
			const txt = await fsp.readFile(filePath, 'utf8');
			...
			await fsp.writeFile(filePath, JSON.stringify({ offerId, updatedAt: new Date().toISOString(), tasks }, null, 2), 'utf8');

			// record history
			try {
				await this.applicationHistoryModel.create({ applicationId: obj.applicationId || null, oldStage: null, newStage: `Preboarding Task Completed: ${tasks[idx].title}`, oldStatus: null, newStatus: null, changedBy: completedBy || null });
			} catch (e) {
				// ignore
			}
	...
	async updateInterview(interviewId: string, updates: Partial<{ scheduledDate: Date | string; method: InterviewMethod | string; videoLink?: string; status?: InterviewStatus | string }>) {
		const doc = await this.interviewModel.findById(interviewId).exec();
		if (!doc) throw new NotFoundException('Interview not found');
		if (updates.scheduledDate) doc.scheduledDate = typeof updates.scheduledDate === 'string' ? new Date(updates.scheduledDate) : updates.scheduledDate as Date;
	...
	async cancelInterview(interviewId: string, opts?: { reason?: string; changedBy?: string }) {
		const doc = await this.interviewModel.findById(interviewId).exec();
		if (!doc) throw new NotFoundException('Interview not found');
	...
	async registerPanelMember(panelMemberId: string, payload: { name: string; email?: string; role?: string; expertise?: string[] }) {
	...
	async setPanelAvailability(panelMemberId: string, availability: Record<string, string[]>) {
	...
	async getPanelCoordinationReport(interviewId: string) {
		...
		const panelMembers: Array<{ member: any; availability: any; feedbackSubmitted: boolean; score?: number }> = [];

		for (const id of panelIds) {
			const member = await this.getPanelMember(id);
			const availability = await this.getPanelAvailability(id);
			// check if this panel member has submitted feedback
			const feedback = await this.assessmentResultModel.findOne({ interviewId: interview._id, interviewerId: id }).lean().exec();
		...
		const avgScore = feedbackCount > 0 ? Math.round((feedbacks.reduce((s: number, f: any) => s + (f.score || 0), 0) / feedbackCount) * 100) / 100 : null;
	...
	async findCommonAvailability(panelMemberIds: string[], dateRange: { startDate: string; endDate: string }) {
	...
	async createChecklist(dto: CreateChecklistDto) {
		...
		return { success: true, checklistId: onboarding._id, taskCount: tasks.length };
	}
	...
	async getContractDetails(contractId: string) {
		...
		if (!offer) throw new BadRequestException('Linked offer not found for this contract');

		const candidate = await this.candidateModel.findById(offer.candidateId).exec();
		if (!candidate) throw new BadRequestException('Linked candidate not found for this offer');

		const fullySigned = !!(contract.employeeSignedAt && contract.employerSignedAt);
		if (!fullySigned) throw new BadRequestException('Contract must be signed by both employee and employer before profile creation');

		if (!candidate.firstName || !candidate.lastName || !candidate.nationalId) {
			throw new BadRequestException('Candidate profile incomplete: missing firstName, lastName, or nationalId');
		}
	...
	async createEmployeeProfileFromContract(contractId: string, createdBy: string) {
	...
	async getTracker(employeeId: string) {
		...
		const onboarding = await this.onboardingModel.findOne({ employeeId: employeeIdObj }).exec();
		if (!onboarding) throw new NotFoundException(`No onboarding found for employee ${employeeId}`);

		const total = onboarding.tasks.length;
		const completed = onboarding.tasks.filter((t: any) => t.status === OnboardingTaskStatus.COMPLETED).length;

		const enrichedTasks = (onboarding.tasks || []).map((t: any, idx: number) => {
			let meta: any = {};
			try {
				if (t.notes && typeof t.notes === 'string' && t.notes.startsWith('{')) {
		...
		return {
			...
			tasks: enrichedTasks,
			progress: { completed, total, percentage: total ? (completed / total) * 100 : 0 },
			nextTask,
	...
	async populateDepartmentTasks(employeeId: string, departmentId?: string) {
		const employeeIdObj = new Types.ObjectId(employeeId);
		const onboarding = await this.onboardingModel.findOne({ employeeId: employeeIdObj }).exec();
		if (!onboarding) throw new NotFoundException(`Onboarding not found for employee ${employeeId}`);

		let dept = departmentId;
		if (!dept) {
			const emp = await this.employeeModel.findById(employeeIdObj).lean().exec();
		...
		const newTasks = deptTemplates.filter((t: any) => !existingNames.includes(t.name));
		...
		this.logger.log(`Department tasks added for employee ${employeeId} (dept: ${dept}). Total tasks: ${onboarding.tasks.length}`);

		return { success: true, tasksAdded: newTasks.length, totalTasks: onboarding.tasks.length };
	}
	...
	async createTask(employeeId: string, dto: CreateOnboardingTaskDto) {
		const employeeIdObj = new Types.ObjectId(employeeId);
		const onboarding = await this.onboardingModel.findOne({ employeeId: employeeIdObj }).exec();
		if (!onboarding) throw new NotFoundException(`Onboarding record not found for employee ${employeeId}`);
		...
		this.logger.log(`Task "${dto.name}" created for employee ${employeeId} (sequence: ${sequence})`);

		return { success: true, task, taskIndex: onboarding.tasks.length - 1, nextReminder: this.calculateNextReminder() };
	}

	async updateTaskStatus(employeeId: string, taskIndex: number, dto: UpdateTaskStatusDto) {
		const employeeIdObj = new Types.ObjectId(employeeId);
		const onboarding = await this.onboardingModel.findOne({ employeeId: employeeIdObj }).exec();
		if (!onboarding) throw new NotFoundException(`Onboarding not found for employee ${employeeId}`);
		if (!onboarding.tasks[taskIndex]) throw new NotFoundException(`Task not found at index ${taskIndex}`);
		...
		if (dto.notes) {
			let meta: any = {};
			try {
				if (task.notes && typeof task.notes === 'string' && task.notes.startsWith('{')) {
		...
		if (dto.status === OnboardingTaskStatus.COMPLETED) task.completedAt = dto.completedAt || new Date();

		if (onboarding.tasks.every((t: any) => t.status === OnboardingTaskStatus.COMPLETED)) {
			onboarding.completed = true;
			onboarding.completedAt = new Date();
		}

		await onboarding.save();
		this.logger.log(`Task ${taskIndex} status updated to ${dto.status} for employee ${employeeId}`);

		return { success: true, status: task.status, completedAt: task.completedAt };
	}
	...
	async sendTaskReminder(employeeId: string, taskIndex: number) {
		...
		const employee = await this.employeeModel.findById(employeeIdObj).lean().exec();
		...
		try {
			...
			this.logger.log(`Task owner reminder created for task "${task.name}" (notification: ${ownerNotif._id})`);
			...
			return {
				...
				taskName: task.name,
				taskOwner,
				employee: employee ? { id: (employee as any)._id, email: (employee as any).workEmail } : null,
			};
		} catch (err) {
			this.logger.error(`Failed to send reminder for task "${task.name}":`, err as any);
			throw new BadRequestException('Failed to send reminder. Please try again.');
		}
	}
	...
	async autoSendRemindersForUpcomingDeadlines() {
		...
		for (const onboarding of onboardings) {
			for (let idx = 0; idx < onboarding.tasks.length; idx++) {
				...
				if (
					...
					const lastReminder = meta.lastReminderSentAt ? new Date(meta.lastReminderSentAt) : null;
					const hoursSinceLastReminder = lastReminder ? (Date.now() - lastReminder.getTime()) / (1000 * 60 * 60) : 24;
					...
					if (hoursSinceLastReminder > 12) {
						try {
							...
							this.logger.error(`Failed to send auto-reminder for task "${task.name}":`, err as any);
						}
					}
				}
			}
		...
		if (remindersSent > 0) {
			await Promise.all(
				onboardings.map((ob) => this.onboardingModel.updateOne({ _id: ob._id }, { tasks: ob.tasks })),
			);
		}

		this.logger.log(`Auto-reminder job completed. Sent ${remindersSent} reminders.`);
	...
	async uploadDocument(dto: UploadOnboardingDocumentDto) {
		...
		const employee = await this.employeeModel.findById(employeeIdObj).lean().exec();
		if (!employee) {
			throw new NotFoundException(`Employee profile not found for ID: ${dto.employeeId}`);
		}
		...
		let onboarding = await this.onboardingModel.findOne({ employeeId: employeeIdObj }).exec();
		let created = false;
		if (!onboarding) {
		...
		try {
			if (created) {
				await this.populateDepartmentTasks((employee as any)._id.toString(), (employee as any)?.primaryDepartmentId?.toString());
			}
		} catch (err) {
			this.logger.warn(`Failed to auto-populate department tasks for employee ${dto.employeeId}: ${err}`);
		}
		...
		try {
			...
			this.logger.log(`Upload notification recorded (to uploader): ${uploaderNotification._id}`);
		} catch (err) {
			this.logger.error('Failed to record notification for document upload', err as any);
		}
	...
	async createProvisionRequest(employeeId: string, dto: CreateProvisionDto) {
		let onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding) {
			onboarding = await this.onboardingModel.create({
		...
		const task = {
			...
			status: OnboardingTaskStatus.PENDING,
			deadline: dto.deadline ? new Date(dto.deadline) : this.calculateDeadline(7),
			notes: JSON.stringify(meta),
		};
	...
	async listProvisionRequests(employeeId: string) {
		const onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).lean().exec();
		if (!onboarding) return { success: true, requests: [] };

		const requests = (onboarding.tasks || [])
			.map((t: any, idx: number) => ({ idx, ...t }))
			.filter((t: any) => typeof t.name === 'string' && t.name.startsWith('Provision:'))
			.map((t: any) => {
				let meta: any = { raw: t.notes };
				try { meta = JSON.parse(t.notes); } catch (e) { meta.raw = t.notes; }
				return { taskIndex: t.idx, name: t.name, status: t.status, deadline: t.deadline, meta };
			});

		return { success: true, requests };
	}

	async approveProvisionRequest(employeeId: string, taskIndex: number, dto: ApproveProvisionDto) {
		const onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding || !onboarding.tasks[taskIndex]) throw new Error('Provisioning request not found');

		const task = onboarding.tasks[taskIndex];
		let meta: any = {};
		try { meta = task.notes ? JSON.parse(task.notes) : {}; } catch (e) { meta.raw = task.notes; }
	...
	async rejectProvisionRequest(employeeId: string, taskIndex: number, dto: RejectProvisionDto) {
		const onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding || !onboarding.tasks[taskIndex]) throw new Error('Provisioning request not found');
		...
		if (dto.notes) meta.notes = (meta.notes ? meta.notes + '\n' : '') + dto.notes;
	...
	async createAccessRequest(employeeId: string, dto: CreateAccessDto) {
		let onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding) {
			onboarding = await this.onboardingModel.create({ employeeId: new Types.ObjectId(employeeId), contractId: null, tasks: [], completed: false });
		}

		const emp = await this.employeeModel.findById(employeeId).lean().exec();
		const requester = dto.requestedBy || (emp ? ((emp as any)._id ? (emp as any)._id.toString() : employeeId) : employeeId);
		const meta = { type: 'access', resource: dto.resource, permissions: dto.permissions || [], requestedBy: requester, createdAt: new Date().toISOString() };
		const task = { name: `Access: ${dto.resource}`, department: dto.department || 'IT', status: OnboardingTaskStatus.PENDING, deadline: this.calculateDeadline(3), notes: JSON.stringify(meta) };
		onboarding.tasks.push(task);
		await onboarding.save();
		try {
			const recipients = await this.employeeSystemRoleModel.find({ roles: { $in: [SystemRole.SYSTEM_ADMIN] }, isActive: true }).lean().exec();
			if (recipients && recipients.length) {
				for (const r of recipients) {
					try {
						...
						this.logger.warn(`Failed to create provision notification for recipient ${r.employeeProfileId}: ${e}`);
					}
				}
			} else {
				this.logger.debug(`No system admin recipients found to notify for provision request (employee ${employeeId})`);
			}
		} catch (err) {
	...
	async approveAccessRequest(employeeId: string, taskIndex: number, dto: ApproveAccessDto) {
		const onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding || !onboarding.tasks[taskIndex]) throw new Error('Access request not found');
		const task = onboarding.tasks[taskIndex];
		let meta: any = {};
		try { meta = task.notes ? JSON.parse(task.notes) : {}; } catch (e) { meta.raw = task.notes; }
		const emp = await this.employeeModel.findById(employeeId).lean().exec();
		meta.approvedBy = dto.approvedBy; meta.grantedTo = dto.grantedTo || (emp ? (emp as any).username || (emp as any).email || null : null); meta.grantedAt = new Date().toISOString(); if (dto.notes) meta.notes = (meta.notes ? meta.notes + '\n' : '') + dto.notes;
		task.status = OnboardingTaskStatus.COMPLETED; task.completedAt = new Date(); task.notes = JSON.stringify(meta);
		await onboarding.save();
		return { success: true, taskIndex, meta };
	}

	async revokeAccess(employeeId: string, taskIndex: number, dto: RevokeAccessDto) {
		const onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding || !onboarding.tasks[taskIndex]) throw new Error('Access task not found');
		const task = onboarding.tasks[taskIndex];
		let meta: any = {};
		try { meta = task.notes ? JSON.parse(task.notes) : {}; } catch (e) { meta.raw = task.notes; }
		const emp = await this.employeeModel.findById(employeeId).lean().exec();
		meta.revokedBy = dto.revokedBy; meta.revokedAt = new Date().toISOString(); if (dto.reason) meta.revocationReason = dto.reason; if (!meta.revocationActor) meta.revocationActor = emp ? ((emp as any).username || (emp as any).email) : null;
		task.notes = JSON.stringify(meta);
		await onboarding.save();
		return { success: true, taskIndex, meta };
	}
	...
	async createEquipmentRequest(employeeId: string, dto: CreateEquipmentDto) {
		...
		const meta = { type: 'equipment', itemType: dto.itemType, preferredModel: dto.preferredModel || null, requestedBy: dto.requestedBy || employeeId, createdAt: new Date().toISOString() };
		const task = { name: `Equipment: ${dto.itemType}`, department: 'Facilities', status: OnboardingTaskStatus.PENDING, deadline: this.calculateDeadline(7), notes: JSON.stringify(meta) };
		onboarding.tasks.push(task);
		await onboarding.save();
		return { success: true, taskIndex: onboarding.tasks.length - 1, task };
	}

	async assignEquipment(employeeId: string, taskIndex: number, dto: AssignEquipmentDto) {
		const onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding || !onboarding.tasks[taskIndex]) throw new Error('Equipment request not found');
		const task = onboarding.tasks[taskIndex];
		let meta: any = {};
		try { meta = task.notes ? JSON.parse(task.notes) : {}; } catch (e) { meta.raw = task.notes; }
		meta.assignedBy = dto.assignedBy; if (dto.assetTag) meta.assetTag = dto.assetTag; if (dto.notes) meta.assignmentNotes = dto.notes; meta.assignedAt = new Date().toISOString();
		task.status = OnboardingTaskStatus.COMPLETED; task.completedAt = new Date(); task.notes = JSON.stringify(meta);
		await onboarding.save();
		return { success: true, taskIndex, meta };
	}

	async returnEquipment(employeeId: string, taskIndex: number, dto: ReturnEquipmentDto) {
		const onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding || !onboarding.tasks[taskIndex]) throw new Error('Equipment task not found');
		const task = onboarding.tasks[taskIndex];
		let meta: any = {};
		try { meta = task.notes ? JSON.parse(task.notes) : {}; } catch (e) { meta.raw = task.notes; }
		meta.returnedBy = dto.returnedBy; if (dto.assetTag) meta.returnedAssetTag = dto.assetTag; if (dto.notes) meta.returnNotes = dto.notes; meta.returnedAt = new Date().toISOString();
		task.notes = JSON.stringify(meta);
		await onboarding.save();
		return { success: true, taskIndex, meta };
	}
	...
	async createPayrollInitiation(employeeId: string, dto: CreatePayrollDto) {
		...
		const meta = { type: 'payroll', payrollType: dto.payrollType, amount: dto.amount || null, frequency: dto.frequency || null, initiatedBy: dto.initiatedBy || employeeId, createdAt: new Date().toISOString() };
		const task = { name: `Payroll: ${dto.payrollType}`, department: 'Payroll', status: OnboardingTaskStatus.PENDING, deadline: this.calculateDeadline(3), notes: JSON.stringify(meta) };
		onboarding.tasks.push(task);
		await onboarding.save();
		return { success: true, taskIndex: onboarding.tasks.length - 1, task };
	}

	async triggerPayroll(employeeId: string, taskIndex: number, dto: TriggerPayrollDto) {
		const onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding || !onboarding.tasks[taskIndex]) throw new Error('Payroll task not found');
		const task = onboarding.tasks[taskIndex];
		let meta: any = {};
		try { meta = task.notes ? JSON.parse(task.notes) : {}; } catch (e) { meta.raw = task.notes; }
		meta.triggeredBy = dto.triggeredBy; if (dto.notes) meta.triggerNotes = dto.notes; meta.triggeredAt = new Date().toISOString();
		task.status = OnboardingTaskStatus.COMPLETED; task.completedAt = new Date(); task.notes = JSON.stringify(meta);
		await onboarding.save();
		return { success: true, taskIndex, meta };
	}
	...
	async createBenefitsRequest(employeeId: string, dto: CreateBenefitsDto) {
		...
		const meta = { type: 'benefits', planType: dto.planType, options: dto.options || [], initiatedBy: dto.initiatedBy || employeeId, createdAt: new Date().toISOString() };
		const task = { name: `Benefits: ${dto.planType}`, department: 'Benefits', status: OnboardingTaskStatus.PENDING, deadline: this.calculateDeadline(5), notes: JSON.stringify(meta) };
		onboarding.tasks.push(task);
		await onboarding.save();
		return { success: true, taskIndex: onboarding.tasks.length - 1, task };
	}

	async approveBenefitsRequest(employeeId: string, taskIndex: number, dto: ApproveBenefitsDto) {
		const onboarding = await this.onboardingModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).exec();
		if (!onboarding || !onboarding.tasks[taskIndex]) throw new Error('Benefits request not found');
		const task = onboarding.tasks[taskIndex];
		let meta: any = {};
		try { meta = task.notes ? JSON.parse(task.notes) : {}; } catch (e) { meta.raw = task.notes; }
		meta.approvedBy = dto.approvedBy; if (dto.enrollmentId) meta.enrollmentId = dto.enrollmentId; if (dto.notes) meta.approvalNotes = dto.notes; meta.approvedAt = new Date().toISOString();
		task.status = OnboardingTaskStatus.COMPLETED; task.completedAt = new Date(); task.notes = JSON.stringify(meta);
		await onboarding.save();
		return { success: true, taskIndex, meta };
	}
	...
	async listPendingDocuments() {
		...
		for (const d of docs) {
			const found = await this.onboardingModel
				.findOne({ 'tasks.documentId': new Types.ObjectId(d._id), 'tasks.status': OnboardingTaskStatus.COMPLETED })
				.lean()
				.exec();
	...
	async verifyDocument(documentId: string, verifiedBy: string, notes?: string) {
		...
		const onboarding = await this.onboardingModel.findOne({ 'tasks.documentId': document._id }).exec();
		if (onboarding) {
			onboarding.tasks = onboarding.tasks.map((t: any) => {
				if (t.documentId && t.documentId.toString() === document._id.toString()) {
					t.status = OnboardingTaskStatus.COMPLETED;
					t.completedAt = new Date();
					let meta: any = {};
					try {
						if (t.notes && typeof t.notes === 'string' && t.notes.startsWith('{')) meta = JSON.parse(t.notes);
			...
			if (onboarding.tasks.every((tt: any) => tt.status === OnboardingTaskStatus.COMPLETED)) {
				onboarding.completed = true;
				onboarding.completedAt = new Date();
			}
			await onboarding.save();
			return { success: true, documentId: document._id, onboardingId: onboarding._id };
		}

		let ownerOnboarding = await this.onboardingModel.findOne({ employeeId: document.ownerId }).exec();
		if (!ownerOnboarding) {
		...
		return { success: true, documentId: document._id, onboardingId: ownerOnboarding._id };
	}

	async rejectDocument(documentId: string, verifiedBy: string, rejectionReason: string, notes?: string) {
		const document = await this.documentModel.findById(documentId).exec();
		if (!document) throw new NotFoundException('Document not found');
	...
	private async generateEmployeeNumber(): Promise<string> {
		...
		const exists = await this.employeeModel.findOne({ employeeNumber: candidateNumber }).lean().exec();
		if (exists) {
	...
	async initiateTerminationReview(
		...
		const performanceData = await this.getEmployeePerformanceData(dto.employeeId);
	 ...
	 */
	async getTerminationReviewsForEmployee(employeeId: Types.ObjectId): Promise<TerminationRequest[]> {
	...
	async getTerminationReviewWithPerformance(
		...
		const performanceData = await this.getEmployeePerformanceData(terminationRequest.employeeId);
		...
		const updatedTermination = await this.terminationRequestModel
			.findByIdAndUpdate(
				terminationId,
				{
					...
					hrComments:
						comments || `System and account access revoked on ${new Date().toISOString()}`,
				},
				{ new: true },
			)
			.exec();
	...
	async updateDepartmentApproval(
		...
		if (itemIndex === -1) {
			throw new NotFoundException(`Department '${department}' not found in checklist`);
		}
	...
	async updateEquipmentReturn(
		terminationId: Types.ObjectId,
		equipmentUpdates: Array<{ name: string; returned: boolean; condition?: string }>,
	): Promise<OffboardingChecklistResponseDto> {
		const checklist = await this.clearanceChecklistModel.findOne({ terminationId }).exec();
	...
	async getOffboardingChecklistSummary(
		...
		const equipmentReturned = checklist.equipmentList.filter((eq: any) => eq.returned).length;
	 ...
	 */
	async completeOffboarding(terminationId: Types.ObjectId): Promise<OffboardingChecklistResponseDto> {
		const checklist = await this.clearanceChecklistModel.findOne({ terminationId }).exec();
		...
		const allEquipmentReturned = checklist.equipmentList.every((eq: any) => eq.returned);
	...
	async updateClearanceStatus(
		...
		if (itemIndex === -1) {
			throw new NotFoundException(`Department '${department}' not found in clearance list`);
		}

		checklist.items[itemIndex].status = status;
		checklist.items[itemIndex].comments = comments || checklist.items[itemIndex].comments;
	...
	async getPendingClearances(): Promise<any[]> {
		...
		return checklists.map((checklist: any) => {
			const pendingItems = checklist.items.filter((item: any) => item.status === 'PENDING');
	 ...
	 */
	async isEmployeeFullyCleared(terminationId: Types.ObjectId): Promise<boolean> {
		const checklist = await this.clearanceChecklistModel.findOne({ terminationId }).exec();
		...
		const items = checklist.items || [];
		return items.length > 0 && items.every((item: any) => item.status === 'APPROVED');
	}
	...
	async sendOffboardingNotification(
		...
		const leaveBalance = await this.getLeaveBalance(terminationRequest.employeeId);
		const benefits = await this.getEmployeeBenefits(terminationRequest.employeeId);

		const notificationData = {
			terminationId,
			employeeId: terminationRequest.employeeId,
			notificationType: dto.notificationType || 'FINAL_PAY_CALCULATION',
			recipientDepartments: dto.recipientDepartments || ['HR', 'Payroll', 'Finance'],
			finalPayCalculation,
			leaveBalance,
	...
	async calculateFinalPay(
		...
		const leaveEncashmentAmount = leaveBalance.balanceLeaveDays * (baseSalary / 30);
	...
	async getOffboardingNotificationHistory(
		...
		const notificationHistory = (terminationRequest as any).notificationsHistory || [];
		const recentNotification = (terminationRequest as any).offboardingNotifications;
	...
	private mapNotificationToResponseDto(notification: any): any {
		return {
			...
			notificationType: notification.offboardingNotifications?.notificationType || 'FINAL_PAY_CALCULATION',
			notificationStatus: notification.offboardingNotifications?.notificationStatus || 'PENDING',
	...
	async submitResignationRequest(
		...
		const resignationData = {
			...
			lastWorkingDay: dto.lastWorkingDay || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
	...
	async getEmployeeResignations(employeeId: Types.ObjectId): Promise<any> {
		...
		const approvedCount = resignations.filter((r: any) => r.status === 'APPROVED').length;
