...
function detectMissedPunch(punches: PunchDto[]): {
  missed: boolean;
  reason: string;
} {
...
function roundTime(
  ...
  if (mode === 'nearest') {
    r = Math.round(timestamp / ms) * ms;
  } else if (mode === 'ceil') {
    r = Math.ceil(timestamp / ms) * ms;
  } else {
    r = Math.floor(timestamp / ms) * ms;
  }
...
function applyHRRulesToPunches(punches: PunchDto[]): PunchDto[] {
  ...
  if (sorted[sorted.length - 1].type === 'OUT') {
    sorted[sorted.length - 1].time = roundTime(
  ...
  for (let i = 1; i < sorted.length - 1; i++) {
    sorted[i].time = roundTime(
      new Date(sorted[i].time),
      snapIntervalThingy,
      'nearest',
    );
  }
...
export class TimeManagementService {
  constructor(
    ...
    private readonly overtimeModel: Model<OvertimeRuleDocument>,
    ...
    @InjectModel('LatenessRule')
    private readonly latenessRuleModel: Model<LatenessRuleDocument>,
    @InjectModel('AttendanceCorrectionRequest')
    private readonly attendanceCorrectionModel: Model<AttendanceCorrectionRequestDocument>,
  ) {}
  ...
  async handleShiftStatus(
    ...
    if (!assignment.endDate) {
      //3ashan yshoof law ongoing :)

      if (assignment.startDate > now) assignment.status = pending;
  ...
  async handleShiftAssignment(
    bro: ShiftAssignmentDocument,
  ): Promise<ShiftAssignmentDocument> {
    const now = new Date();

    if (bro.status === ShiftAssignmentStatus.CANCELLED) return bro;

    if (!bro.startDate || !bro.endDate)
      throw new BadRequestException(
        'Shift assignment must have startDate and endDate',
      );
    ...
    if (bro.endDate < now) bro.status = ShiftAssignmentStatus.EXPIRED;
    else if (bro.startDate > now) bro.status = ShiftAssignmentStatus.PENDING;
    else bro.status = ShiftAssignmentStatus.APPROVED;
    return bro;
  }
  ...
  async getShift(id: string): Promise<ShiftDocument> {
    ...
    if (!g) throw new NotFoundException('Shift with id ${id} not found');
    return g;
  }
  ...
  async createShift(dto: CreateShiftDto): Promise<ShiftDocument> {
    ...
    if (
      ...
      toMinutes(newShift.startTime) >= toMinutes(newShift.endTime)
    )
  ...
  async updateShift(id: string, dto: UpdateShiftDto): Promise<ShiftDocument> {
    ...
    if (!u) throw new NotFoundException('Shift with id ${id} not found');
    return u;
  }
  ...
  async deleteShift(id: string): Promise<ShiftDocument> {
    ...
    if (!d) throw new NotFoundException('Shift with id ${id} not found');
    return d;
  }
  ...
  async cancelShiftAssignment(id: string): Promise<ShiftAssignmentDocument> {
    ...
    if (!bro) throw new NotFoundException('Shift assignment not found');
  ...
  async getShiftAssignmentByDepartment(
    d: string,
  ): Promise<ShiftAssignmentDocument[]> {
  ...
  async getShiftAssignmentByEmployee(
    d: string,
  ): Promise<ShiftAssignmentDocument[]> {
    const s = await this.shiftAssignmentModel
  ...
  async getShiftAssignmentByPosition(
    d: string,
  ): Promise<ShiftAssignmentDocument[]> {
    const s = await this.shiftAssignmentModel
  ...
  async createShiftAssignmentByDepartment(
    shiftData: CreateShiftAssignmentDtoDepartment,
  ): Promise<ShiftAssignmentDocument> {
    let bro = new this.shiftAssignmentModel(shiftData);
    if (!bro)
      throw new NotFoundException(
        'Could not create shift assignment for this employee',
      );
    this.handleShiftAssignment(bro);
    return bro.save();
  }

  async createShiftAssignmentByEmployee(
    shiftData: CreateShiftAssignmentDtoEmployee,
  ): Promise<ShiftAssignmentDocument> {
  ...
  async createShiftAssignmentByPosition(
    shiftData: CreateShiftAssignmentDtoPosition,
  ): Promise<ShiftAssignmentDocument> {
    const bro = new this.shiftAssignmentModel(shiftData);
    if (!bro)
      throw new NotFoundException(
        'Could not create shift assignment for this position',
      );
    this.handleShiftAssignment(bro);
    return bro.save();
  }
  ...
  async updateShiftAssignmentByDepartment(
    id: string,
    updateData: UpdateShiftAssignmentDtoEmployee,
  ): Promise<ShiftAssignmentDocument[]> {
    ...
    const updatedDocs = await this.shiftAssignmentModel.find({
      departmentId: new Types.ObjectId(id),
    });
  ...
  async updateShiftAssignmentByEmployee(
    id: string,
    updateData: UpdateShiftAssignmentDtoEmployee,
  ): Promise<ShiftAssignmentDocument[]> {
  ...
  async updateShiftAssignmentByPosition(
    id: string,
    updateData: UpdateShiftAssignmentDtoPosition,
  ): Promise<ShiftAssignmentDocument[]> {
  ...
  async deleteShiftAssignmentByEmployee(
    id: string,
  ): Promise<ShiftAssignmentDocument> {
    ...
    const deleted = await this.shiftAssignmentModel.findOneAndDelete({
      employeeId: new Types.ObjectId(id),
    });

    if (!deleted)
      throw new NotFoundException('Shift not found for this employee');

    return deleted;
  }

  async deleteShiftAssignmentByPosition(
    id: string,
  ): Promise<ShiftAssignmentDocument> {
    if (!Types.ObjectId.isValid(id)) {
    ...
    const deletedShift = await this.shiftAssignmentModel.findOneAndDelete({
      positionId: new Types.ObjectId(id),
    });

    if (!deletedShift) {
      throw new NotFoundException('Shift not found for this position');
    }

    return deletedShift;
  }

  async deleteShiftAssignmentByDepartment(
    id: string,
  ): Promise<ShiftAssignmentDocument> {
    if (!Types.ObjectId.isValid(id)) {
    ...
    if (!deletedShift) {
      throw new NotFoundException('Shift not found for this department');
    }
  ...
  async updateSchedule(
    id: string,
    updateData: UpdateScheduleRuleDto,
  ): Promise<ScheduleRuleDocument> {
    if (!mongoose.Types.ObjectId.isValid(id))
      throw new NotFoundException('Schedule with id ${id} not found');

    const updated = await this.scheduleRuleModel.findByIdAndUpdate(
      id,
      updateData,
      { new: true },
    );
    if (!updated) {
      throw new Error('Scheuld not found');
    }
  ...
  async updateShiftType(
    ...
    if (!mongoose.Types.ObjectId.isValid(id))
      throw new NotFoundException('ShiftType with id ${id} not found');
    const updatedShiftType = await this.shiftTypeModel.findByIdAndUpdate(
      id,
      data,
      { new: true },
    );
  ...
  async getOvertimeRule(id: string): Promise<OvertimeRuleDocument> {
    if (!mongoose.Types.ObjectId.isValid(id))
      throw new NotFoundException('Overtime thing with id ${id} not found');
  ...
  async getOvertimeRuleCalculation(
    type: OvertimeRuleDocument,
    employeeId: string,
  ): Promise<number> {
    if (!type)
      throw new NotFoundException('Overtime thing with id ${id} not found');
    ...
    if (!attendance)
      throw new NotFoundException(
        'I hate seeing errors brdo, Employee has no attendance???',
      );
  ...
  async getEmployeeAttendanceRecord(
    employeeId: string,
  ...
  async createOvertimeRule(
    data: CreateOvertimeRuleDto,
  ): Promise<OvertimeRuleDocument> {
  ...
  async updateOvertimeRule(
    ...
    if (!mongoose.Types.ObjectId.isValid(id))
      throw new NotFoundException('Overtime thing with id ${id} not found');
    const updated = await this.overtimeModel.findByIdAndUpdate(id, data, {
      new: true,
    });
    if (!updated)
      throw new NotFoundException('Overtime thing with id ${id} not found');
  ...
  async createAttendance(
    ...
    if (data.punches) data.punches = applyHRRulesToPunches(data.punches);
  ...
  async updateAttendance(
    ...
    if (data.punches)
      //read the thing above - present me has no idea why past me wrote this
  ...
  async createLateness(
    data: CreateLatenessRuleDto,
  ): Promise<LatenessRuleDocument> {
    const record = new this.latenessRuleModel(data);
    return record.save();
  }

  async updateLateness(
    id: string,
  ...
  async getAllAttendanceCorrection(): Promise<
    AttendanceCorrectionRequestDocument[]
  > {
    return this.attendanceCorrectionModel.find().exec();
  }

  async getAttendanceCorrection(
    id: string,
  ): Promise<AttendanceCorrectionRequestDocument> {
    if (!mongoose.Types.ObjectId.isValid(id))
      throw new NotFoundException(
  ...
  async updateAttendanceCorrection(
    id: string,
    data: UpdateAttendanceCorrectionRequestDto,
  ): Promise<AttendanceCorrectionRequestDocument> {
    if (!mongoose.Types.ObjectId.isValid(id))
      throw new NotFoundException(
        'attendance correction with id ${id} not found',
    ...
    const updated = await this.attendanceCorrectionModel.findByIdAndUpdate(
      id,
      data,
      {
        new: true,
   ...
   *
   * @param periodDate - The date in the month to calculate
  ...
  async getWorkDaysInMonth(periodDate: Date): Promise<number> {
    ...
    while (currentDate <= lastDay) {
      ...
      // Check if it's not a weekend (Friday=5, Saturday=6 for Egypt)
      // and not a holiday
      if (dayOfWeek !== 5 && dayOfWeek !== 6 && !holidayDates.has(dateString)) {
        workDays++;
      }
  ...
  /**
   * Get employee overtime hours for a period
   * Used by payroll execution for overtime compensation
   *
   * @param employeeId - Employee ID
   * @param periodDate - End date of payroll period
  ...
  /**
   * Get employee absence days for a period
   * Used by payroll execution for penalty calculations
   *
   * @param employeeId - Employee ID
   * @param periodDate - End date of payroll period
   * @returns Number of absence days
   */
  async getEmployeeAbsenceDays(
    employeeId: string,
    periodDate: Date,
  ): Promise<number> {
    const year = periodDate.getFullYear();
    ...
    // Get attendance records with absences
    const records = await this.attendanceRecordModel
      .find({
        employeeId: new mongoose.Types.ObjectId(employeeId),
        date: {
        ...
        status: 'absent', // Adjust based on your enum
      })
      .exec();
