...
import {
  createShiftAssignmentByDepartment,
  getAllSchedule,
  getAllShifts,
} from "../api/index";
...
export default function CreateShiftAssignmentDepartmentForm({
  onCreated,
}: CreateShiftAssignmentDepartmentFormProps) {
  const [shifts, setShifts] = useState<Shift[]>([]);
  const [scheduleRules, setScheduleRules] = useState<ScheduleRule[]>([]);
  const [departments, setDepartments] = useState<EmployeeProfile[]>([]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!selectedDepartmentId || !selectedShiftId || !startDate) {
      setError("Department, Shift, and Start Date are required");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      await createShiftAssignmentByDepartment(shiftAssignmentData);
    ...
    <form onSubmit={submit} className={s.formContainer}>
      ...
      <div className={s.grid}>
        <div className={s.field}>
          <label className={s.description}>Department</label>
          <select
            className={s.select}
